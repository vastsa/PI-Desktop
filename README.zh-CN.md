...
PI-Desktop 处于活跃开发中的早期预览阶段。当前 0.11.x 已交付：应用外壳、流式智能体运行时、智能体/规划/目标合约、带权限系统的工作区工具、工作台、项目与会话、会话导入、智能体能力管理（MCP/Skills/Subagents）、后台子智能体委派、基于自动发现与 models.dev 模型目录的服务商配置、单服务商多模型与 OAuth 账号、扩展（插件）与插件市场、上下文检查点、通知、应用内更新日志，以及带更新分发的跨平台打包。
