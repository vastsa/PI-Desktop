...
function buildInstruction({ topic, agents, rounds, goal }) {
  ...
  return [
    ...
    `an existing one that already declares the Peer tool).`,
    `Each definition must include the tools: Peer.`,
    ``,
    `Roles:`,
    peerList,
    ``,
    ...
    `>    role's perspective. Peer(action="send") your opening statement to all other`,
    `>    participants.`,
    `> 2. **Discussion rounds (${numRounds} total)** — At the start of each round:`,
    `>    a. Call Peer(action="inbox") to read messages from other participants.`,
    `>    b. Respond to specific points raised by others. Peer(action="send",`,
    `>       to="<peer>") when replying directly; broadcast for general observations.`,
    ...
    `>    unresolved concerns. Peer(action="send") your summary to all peers, then return`,
