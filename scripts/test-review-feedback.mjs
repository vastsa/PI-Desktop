/** Review-to-composer user paths in isolated Electron/Chromium.
...
const temp = await mkdtemp(join(tmpdir(), "pi-review-feedback-"));
const artifacts = process.env.PI_REVIEW_ARTIFACT_DIR ? resolve(process.env.PI_REVIEW_ARTIFACT_DIR) : temp;
...
await build({ entryPoints: [join(root, "apps/desktop/test/fixtures/review-feedback.jsx")],
  outfile: join(temp, "fixture.js"), bundle: true, format: "esm", platform: "browser", jsx: "automatic",
  plugins: process.argv.includes("--baseline") ? [{ name: "baseline-review", setup(build) {
    build.onLoad({ filter: /[/\\]ReviewChangeCard\.tsx$/ }, () => ({ contents: execFileSync("git", ["show", "HEAD:apps/desktop/src/components/ReviewChangeCard.tsx"], { cwd: root, encoding: "utf8" }), loader: "tsx" }));
  } }] : [],
...
const env = { ...process.env, PI_REVIEW_BASELINE: process.argv.includes("--baseline") ? "1" : "0", PI_REVIEW_FIXTURE_URL: url, PI_REVIEW_ARTIFACT_DIR: artifacts }; delete env.ELECTRON_RUN_AS_NODE;
try {
  const child = spawn(resolveElectronBinary(root).electronBinary, [`--user-data-dir=${join(temp, "profile")}`, join(root, "apps/desktop/test/fixtures/review-feedback-runner.cjs")], { env, windowsHide: true, stdio: "inherit" });
