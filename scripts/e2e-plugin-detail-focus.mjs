#!/usr/bin/env node
// Real marketplace components and Chromium focus; only host APIs are fixtures.
...
const cache = join(root, ".cache/plugin-detail-focus");
await mkdir(cache, { recursive: true });
const temp = await mkdtemp(join(cache, "run-"));
await require("esbuild").build({
  entryPoints: [join(root, "scripts/e2e/plugin-detail-focus.tsx")],
...
app.whenReady().then(async()=>{
  try {
    ...
    console.log(await win.webContents.executeJavaScript('globalThis.pluginDetailFocusProbe()'));
    app.exit(0);
