...
other capability behind the local `tool_search` tool, so a large plugin surface
...
2. The tool stays out of the deferred catalog and therefore never appears under
   `# On-demand tools`. Other on-demand capabilities (`browser_preview`, plugin
   tools, plugin-development helpers, and MCP tools) keep their lazy behavior,
   and `tool_search` is still registered whenever any of them exist.
...
- Removing the `skill` row from the deferred catalog also removes the only path
  that could route a skill through `tool_search`, so the catalog description for
