...
## 2026-09-22 —— 工具名只有一个规范拼写与一个归一化边界（D618）

- 模型可见的工具名此前是界面展示的大写拼写（`Read`、`Bash`、`TaskWait`），而 pi 运行时在
  `extractFileOpsFromMessage` 这类辅助函数里按自己的小写名分支。pi 不认识的名字会静默地什么
  都不产出——这正是 `details.readFiles`、`details.modifiedFiles` 以及摘要的 `<read-files>`
  段恒为空的成因，也是 issue #827 在修复压缩回退时踩到的线索。
- 宿主与 shared 包现在各自持有一份规范名清单和一个纯归一化函数
...
- 这些边界都是"读入时归一化"，不写回存储：`tools.execute` 在入口处对 `toolName` 归一化一次
  ...
  因为导入档案属于写路径。
- 由 host-core 自己定义的四个计划任务工具（`scheduled_task_list` / `_create` / `_update` /
  `_delete`）连同其 schema、参数白名单、`risk` 分支与派发一并改名，且
  `scheduled_tools::recognizes` 也对自己收到的名字归一化——它是调用方直接查询的门，而不是派发器内部
  的形式步骤。
...
- 回归测试贴在各自边界旁（`legacy_tool_names_dispatch_to_the_same_builtin`、
  ...
  `legacy_scheduled_task_names_dispatch_to_the_same_tools`），另有两例 RPC 用例分别回放旧名执行与
  预览权限判定。
