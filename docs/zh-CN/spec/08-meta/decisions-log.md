...
| D433 | Deny-first 权限规则 | **修订 D115：权限模式仍为 `ask` / `accept-edits` / `auto`。叠加用户设置 `AppSettings.permissionDeny` ∪ 已启用、已授予 `agent.permission.deny`、且激活范围命中会话工作区的插件贡献。Schema 为 `{ tools, paths, commands }` glob；未知键拒绝；每列表最多 256 条、每条最多 512 字符；空 `{}` 合法。命中为 `PermissionDecision::Deny`，位于 Plan/Goal 硬拒绝之后、外部路径 / 低风险 / auto / grants 之前。插件只能加规则。命中沿用 `TOOL_DENIED`。见 ADR 0267。** | `auto` 等价于无界 YOLO。用户与策略插件需要一份始终拒绝的名单，且不能被插件或会话授权放宽。 |
...
## 2026-09-15 —— Deny-first 是叠加而非第四种模式（D433）
