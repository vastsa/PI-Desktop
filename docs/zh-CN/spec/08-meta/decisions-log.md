...
| D443 | 规范应用 ID 与 macOS 代码签名标识 | **修订 D141 / D371 / ADR 0204：应用 ID 为 `net.aiuo.pi-desktop`（`APP_ID`、electron-builder `appId`、macOS `CFBundleIdentifier`、Windows AppUserModelID）。开发用 macOS 宿主为 `net.aiuo.pi-desktop.dev`。未签名 macOS 打包后，对外层应用做 adhoc 签名并使 Identifier 等于该 ID，不得残留 `Electron`。不要 `--deep`。Developer ID 签名若 Identifier 不匹配则打包失败。见 ADR 0278、E2E-196d、issue #524。** | `usernotificationsd` 会拒绝 codesign Identifier 为 `Electron` 而 plist 为产品 Bundle ID 的通知请求。所有者域名为 `net.aiuo.pi-desktop`。 |
