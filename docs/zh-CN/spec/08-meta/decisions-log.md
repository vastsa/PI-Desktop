...
| D163 | 固定工作面板的原生宽度预留（由 D287 取代） | *（历史宽度范围由 D167 取代）* **开放式对接面板在 `364..720` 中保留一个承诺的固定宽度；本机 window/sidebar 更改永远不会限制它。 Renderer 通过幂等`window/setWorkPanelReservation({width: 0 | 364..720}) -> {请求，保留}`: open requests the committed width, collapse/final close requests zero, and divider commit updates it. In normal state Main adds available native work-area width and reverses it symmetrically, so chat stays stable when the full target fits; otherwise the panel remains fixed and chat absorbs `请求 - 保留`。本机边缘仅调整聊天大小。 Maximized/fullscreen 请求推迟到正常； display/work-area 更改可根据当前可用宽度协调目标，而无需在普通的同一显示移动期间重新应用它。持久基址边界不包括预留宽度及其 x 偏移，并且背景伪影无法更改可见预留。这取代了 ADR 0029/D156 子句，这些子句将面板固定在当前客户端宽度内或禁止面板驱动的目标几何图形，同时保留主边界所有权和分隔符手势规则 (ADR 0032)。** | 当显示可以扩展正常窗口时，停靠的工具不应占用聊天的宽度，并且调整聊天大小不应挤压该工具。有界目标状态保留提供类似 Codex 的行为，无需恢复非幂等增量或循环偏好更新。 |
| D167 | 更窄的默认工作面板宽度 | **停靠的工作面板以 280px 承诺默认值打开（比 D163 的 420px 窄三分之一）并固定到 `244..720px`；渲染器常量、Electron 预留验证器和 `.work-panel` CSS 底层共享这些边界。双击分隔线可恢复 280 像素。由于工具位于标题切换器中，D154 的 364px 底面背后的 44px-rail-plus-320px-content 基本原理不再适用，因此底面按默认比例缩放。持续更宽的宽度仍然有效，并且在升级时保持不变；最大 720px 与 renderer divider 手势规则保持不变；D287 覆盖原生 reservation 生命周期（ADR 0148）。** | 该面板的打开范围比大多数 review/terminal/file 内容需求更宽，从固定客户区域内的聊天中获取可读宽度，而旧的 364px 地板使得较窄的默认值无法到达。 |
| D255 | 工作面板可见期间恢复原生预留（由 D287 取代） | **历史决定：打开 in-flow 面板时请求同宽原生 reservation，使 BrowserWindow 扩大而 MainChat 保持稳定。** | D287 恢复固定窗口内部停靠，因为面板应在现有应用框架内缩窄会话区。 |
| D287 | 固定窗口工作面板动画与持久 toggle | **右侧工作面板保持 in-flow flex sibling，但始终请求原生 reservation `0`：从窗口右边缘动画 width/flex-basis，MainChat 在 BrowserWindow 边界不变时连续缩窄。AppShell 在每个非 Settings 路由的顶栏带拥有一个固定 toggle，打开和关闭状态下视口位置不变；工作面板标题不含重复折叠控件。原生窗口边缘正常调整窗口，内部 divider 调整 244..720px 面板宽度。该决定取代 D255 及 D286 / ADR 0146 的右边缘面板目标条款。** | 与 Codex 一致：应用框架和触发按钮保持稳定，会话区为滑入面板让出宽度。 |
| D288 | Renderer 生命周期的会话 Composer 草稿 | **未发送文本和文件引用按会话缓存在 renderer 模块生命周期。挂载恢复活动 key；会话切换和卸载通过 live refs 保存，不做逐按键序列化。成功发送只清来源 key，失败保留，删除会话会清理。页面、项目及空态/对话态切换后返回均恢复；renderer 重载或应用重启清除。无 host schema、IPC、localStorage 或持久提示正文变更。** | 实例级 Map 会在 ChatSurface 或 Composer 重挂载时消失，导致未发送输入丢失。 |
