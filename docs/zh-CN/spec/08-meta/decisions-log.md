...
| D420 | Deny-first 权限规则 | **修订 D115：权限模式仍为 `ask` / `accept-edits` / `auto`。叠加用户设置 `AppSettings.permissionDeny` ∪ 已启用、已授予 `agent.permission.deny`、且激活范围命中会话工作区的插件贡献。Schema 为 `{ tools, paths, commands }` glob；未知键拒绝；每列表最多 256 条、每条最多 512 字符；空 `{}` 合法。命中为 `PermissionDecision::Deny`，位于 Plan/Goal 硬拒绝之后、外部路径 / 低风险 / auto / grants 之前。插件只能加规则。命中沿用 `TOOL_DENIED`。见 ADR 0249。** | `auto` 等价于无界 YOLO。用户与策略插件需要一份始终拒绝的名单，且不能被插件或会话授权放宽。 |
...
## 2026-09-15 —— Deny-first 是叠加而非第四种模式（D420）

- 权限模式仍为 `ask` / `accept-edits` / `auto`。`auto` 仍是高风险工具的 YOLO 等价物，
  但不再无界：用户自己的 `AppSettings.permissionDeny` 与已授权插件贡献可以点名
  始终拒绝的工具、路径和命令前缀。
- 叠加是用户 JSON 与已启用、持有 `agent.permission.deny`、且激活范围命中会话工作区的
  插件 `contributes.permissionDeny` 的并集。插件只能 deny；没有 `allow` 键，也不能删除用户规则。
- 求值顺序冻结：Plan/Goal 硬拒绝，然后 deny-first，然后外部路径 / 低风险 / auto / grants / 确认卡片。
  命中沿用既有 `TOOL_DENIED`，不把命中的 glob 教给模型。
- 设置 → AI → 权限在模式选择器下提供 JSON。Schema 为 `{ tools, paths, commands }`，
  未知键拒绝，每列表 256 条 × 每条 512 字符。SDK 校验形状；host-core 用 `globset` 编译 glob。
- 见 ADR 0249。
