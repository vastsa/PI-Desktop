...
单条限制约束一个结果，压力闸门约束结果集合。当模型绑定启用 `dynamicContext`（ADR 0301）且出站视图达到硬限制的阈值份额时，
旧工具结果被缩成「头部 + 指向如何读回全文的指针」——被丢弃的文本仍可通过 `recall` 读回，因此窄化不销毁数据。

- 闸门与出站请求、硬边界判定、空闲压缩测的是同一个视图；
- 会话重新打开过的文件的结果保持完整（工作集免疫），被排除的工具族与模块自身的下限同样豁免；
- 低于阈值或闸门关闭时，视图原样返回；
- 已存转录行永不被改写，被截断的结果仍带标记。
...
- [ ] 压力闸门只在阈值之上窄化旧工具结果，重新打开过的文件结果保持完整，全文仍可通过指针读回
