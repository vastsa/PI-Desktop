...
**边界可回溯。** 完整转录与模型上下文是两个视图，压缩只作用于后者：任何检查点都不会改写、删除或隐藏已存的行。因此投影会说明
模型仍能读到什么：`RECALL_POINTER` 指向两个 recall 工具，检查点的 `details.ledger`（由 `buildLedger` 生成）记录该边界覆盖的
机械事实——读过与改过的文件、命令、消息与工具调用计数、目标与未解决项——以有界区块渲染在摘要之后。早于 ledger 的检查点只投影
指针。摘要生成失败时，降级检查点用同一个助手描述自身覆盖范围，且不会为已完成的回合持久化空尾部（ADR 0300）。

**空闲预压缩。** 当模型绑定启用 `earlyCompaction`（ADR 0301）时，运行时在一次 run 结束后武装后台压缩：只有在 `delaySeconds`
内没有新活动才触发；任何新提示都会取消计时器或让本次让位，且该通道在花掉摘要请求之前会重新核对预算，因此窄化出足够空间即可
取消它。它发出的 `compaction_end` 与内联路径相同，另带 `idle: true`；成功且设置为静默时带 `silent: true`，渲染层跳过例行
toast。转录行、检查器与检查点记录照常写入；降级或失败以及硬边界始终告警。内联路径不可配置、行为不变。

**可配置压力闸门与工具结果降级。** 绑定上的 `dynamicContext` 决定在硬限制的哪个份额开始把旧工具结果缩成「头部 + 恢复指针」
（`narrowToolResults`，`tool-result-tier.ts`）；出站请求、硬边界判定与空闲压缩读同一个视图，所以窄化省下的量对压缩可见。
会话重新打开过的文件的结果保持完整，单条工具限制（规格 16）不变，闸门关闭时不窄化。睡眠摘要（`sleepTime`）在摘要请求之前写入
一条确定性的 `sleep` 转录行：不调用提供商，默认关闭。
