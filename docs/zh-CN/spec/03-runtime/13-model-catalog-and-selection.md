...
新会话按“显式草稿 > 本机最近主动选择且仍可用的供应商/模型 > 全局默认”确定模型。
记忆模型必须仍在启用且已认证的供应商配置中；失效时回退全局默认，不使用更早的记忆。
思考等级按供应商和模型分别记忆，恢复时校验当前可用档位；`omit` 仅对推理模型保留。
记忆使用版本化的本机渲染器偏好，最多保存 100 对，不修改全局设置或已有会话。
仅模型/思考菜单的主动操作更新记忆；浏览历史、分叉、固定旧会话模型、模式及权限变更不更新。
配置失败不更新；运行中接受的下一轮配置会更新；旧异步请求不能覆盖更新的选择。
存储损坏或不可用时安全回退。权限仍继承全局默认。

没有记忆时，渲染器会解析所选模型的 `ModelBinding`。
...
Composer 选择（或全局默认回退）写入持久化 id。之后改默认模型不会改写已创建
...
会话级别：
- 创建时按显式草稿、最近可用主动选择、全局默认的顺序写入 `providerId`/`modelId`
- 全局默认仍是无可用记忆时的回退，不改已创建会话
- 优先恢复该供应商/模型的思考等级记忆，否则使用模型绑定默认等级（钳位到已启用档；
...
- [ ] 能力徽章可见
- [ ] 会话模型更改仅适用于下一回合
- [ ] 新会话优先恢复该供应商/模型的思考记忆，否则使用绑定存储的默认
