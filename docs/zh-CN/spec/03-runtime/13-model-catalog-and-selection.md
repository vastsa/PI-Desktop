...
图标由解析得到的目录 key 决定，绝不依据显示名猜测：该 key 由主进程经同一套已用于
定位行元数据的别名映射给出，因此被用户改名为同一名称的两个提供商仍各自持有自己的
图标，自定义名称也不会借用某厂商的美术资源。

行优先使用自己的 `catalogVendorKey`，其次才是提供商的 `catalogProviderKey`——因为
一个自定义端点往往同时服务多个厂商：同一组行共享提供商 key，而每一行各自指名其归
属。该归属从模型 ID 的厂商路由（`openai/gpt-6-astra` 指名 `openai`）读出，扫描范围是
目录为该 ID 自带的候选索引（即 `findModel` 所用的同一套索引），再经既有的提供商别
名表映射。它指名的是权重归属者，而不是本次查找恰好命中的那条记录由谁发布；它只是元
数据，不改变任何能力、上限或线模型 ID。

模型 ID 不含厂商路由时，改由其发布者指认归属：凡以自身 key 发布该模型的厂商（例如
小米以自己的 key 发布 `mimo-v2.6-pro`），即该厂商在描述自家模型。中转网关不符合这一
条件——因为它的 key（`opencode-go`、`nano-gpt`、`requesty`）不在厂商集合里，所以只是
转载该模型的网关无法冒领其图标。厂商路由始终优先于发布者 key，因为模型 ID 比列出它
的主机更直接地说明了归属。

两种信号都指认不出厂商的模型 ID（包括未知的自由格式 ID）解析不出任何结果，于是回退。

图标随发布版本打包为单色 `currentColor` 美术资源，由
`scripts/build-provider-marks.mjs` 编译为内联组件。运行时不从图标站点拉取任何资源，
因此选择器在离线状态下外观一致，也不会有外发请求泄露用户配置了哪些提供商；它们是
路径而非图片，因此没有会失败的 `<img>` 元素，也不占 CSP 表面。继承主题文字颜色使
各厂商保持同一视觉重量，而不是在列表中堆一排高饱和度的彩色图标。见 ADR
