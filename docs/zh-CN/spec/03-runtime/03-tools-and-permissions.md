...
### 2.2 Grep 工作区索引快路径（可选，默认关）

当 `indexGrepBoost` 为 false 或缺省时，Grep 行为与索引引入之前逐字节一致。
用户开启后，在 `fresh` 索引 root 上，区分大小写的字面量 pattern（无正则元字符、
至少 3 个码点）可由索引服务：索引只缩小候选文件列表，输出仍由同一逐行扫描器与
结果预算产生，结果形状不变。正则、短模式、大小写不敏感查询始终全量走查。
逐候选新鲜度校验在漂移时回退常规走查。索引可见集与 Grep 可见集由
`tools/ignore_rules` 与 diff 属性测试保证相等。
