...
`generate_images({items: [{prompt, count?, images?}]})` 仅供 Agent 模式使用。`count` 默认为 1，支持不同提示词和同一提示词的多个变体，总输出为 1–10 张。每项可通过 `images` 提供 1–4 张本地参考图，之前生成的路径可用于迭代编辑。提示词最多 32,000 个字符；不支持或超限输入直接拒绝，不截断。Plan 和 Goal 不得执行此工具。
