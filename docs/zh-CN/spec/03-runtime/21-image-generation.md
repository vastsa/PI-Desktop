...
成功结果显示在一张圆角图片卡片中。同一批次有多张成功图片时，主图旁显示缩略图列；选择缩略图会切换主图和对应下载目标，失败项仍单独显示。窄窗口中缩略图仍可操作。点击主图后打开全窗口预览，支持批次缩略图、方向键切换、缩放与适应窗口，也可返回现有文件面板。切换图片时不会在新选中状态下短暂显示上一张；已加载图片之间切换时，卡片和全窗口预览持续显示图片，不闪出空白占位。预览在下一张图片解码期间保持适配尺寸，快速连续操作以最后一次选择为准。选中缩略图使用浅灰色外框；关闭预览后，对话卡片保留选中的图片。卡片和预览都可从受限图片数据下载，文件名根据图片类型安全生成；预览中的“在文件夹中显示”通过受限 `fs/reveal` 接口执行。预览缺失或不可读时不提供下载，定位失败会提示错误；这些操作不会移动或覆盖 scratch 中的原图。
