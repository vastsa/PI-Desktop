...
| `PROVIDER_ERROR` | 是的 | 上游提供商故障；可重试的故障（5xx 网关）最多获得三次同回合重试，格式错误的 400/422 请求是终止的 |
...
| `STREAM_FAILED` | 是的 | 提供程序流在完整响应之前终止、提前关闭或以其他方式结束；最多三次同回合重试可能会在终止事件之前发生 |
...
502/503/504）——共享它们自己的有界预算：初始尝试之后最多三次重试，同样
...
上限为 8 秒。来自格式错误的 400/422 请求的不可重试 `PROVIDER_ERROR` 永远
不会进入任何预算。预算耗尽后的失败仍然是致命的。
