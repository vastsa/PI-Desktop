...
- 两种模式都为每个已加载的助手回合提供一个整体过程披露，其中包含思考、工具、托管
  搜索和中间进度文字；末尾回答、助手错误和中止后的末尾文字位于过程之外。当回合仍在
  运行且过程中已有更早的工具调用或托管搜索轮次时，这段末尾文字按过程缩进与次要色调
  呈现为过渡性叙述，并位于可折叠正文之外；该呈现只由回合状态决定，文字停止流式输出后
  移除。
- 连续活动片段仅在当前模式下至少有两个可见项时获得组披露。进度文字结束该片段，
  单项活动直接使用自身披露，紧凑模式隐藏的思考不会制造多余组；Task 拓扑保持独立。
- 详细模式下，活动回合的整体过程保持展开，回合完成后（运行状态已结束，末尾答案
  `status` 为 `complete`、无错误、无中止、内容非空）在用户未操作时自动收起；手动或
  搜索触发的展开会保留。活动普通组默认展开，完成时仅在用户未操作的情况下收起；其他
  已完成组默认收起。紧凑模式下过程与组默认收起，但活动回合
...
- 手动操作条目会把所属组和过程标记为用户接管，但不会切换祖先。流式更新和完成不会
  重新打开手动收起，也不会收起包含用户已展开、聚焦或选中内容的容器。完成折叠这种
  无人要求的收起会把标题交给滚动器：对已离开末尾的读者保持标题位置，对仍贴底跟随的
  读者则跳过该保持，因为此时固定收起后的标题会把回答拖出视口。只要保留会话
...
- 固定的转录在内容或视口尺寸变化的同一帧内重新贴底，绝不延后一帧。
  ...
  滑到输入框后面（D287）。
- 切换整个过程、活动组、工具／搜索／思考条目、委派摘要或错误详情时，由所属滚动器
  保持阅读位置（issue #324）。只有发起切换的层级声明滚动锚点；把祖先标记为用户接管
  不会同时声明祖先位置。状态变化前先把标题交给滚动器，并在高度变化的每一帧恢复其
  视口偏移。完成折叠是唯一的自动收起，也采用同样的保持，但对仍贴底跟随的读者例外：
  该读者继续固定在回答上。嵌套滚动器（委派运行停靠区，D302）保持自身位置，并因外层
  内容同时增长而向外传递保持。
