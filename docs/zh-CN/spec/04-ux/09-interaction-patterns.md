...
- 在单条用户消息或助手正文中选中文字，选区旁直接显示“添加到会话”。点击后
  在当前会话的输入框上方显示数量卡片，可预览、逐条移除所选原文；可编辑草稿和
  文件引用保持不变。发送时原文作为引用上下文加入请求，失败后恢复卡片，点击
  操作本身不自动发送。清除选区、滚动、调整窗口或切换面板会关闭按钮；工具输出、
  思考内容和跨消息选区不显示按钮。右键菜单仍提供整条消息的入口（ADR 0307）。
