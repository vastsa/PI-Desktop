...
- 10px 左边缘分隔符锚定到按下位置和起始宽度，然后跟随指针 delta 而不跳转。向左拖动会持续增长面板，直到共享预算耗尽；MainChat 到达 360px 下限时展开的左栏立即收起。向右拖动把空间还给 MainChat。
- 分隔线目标钳制在三栏共享预算内（`min(720px, 客户端宽度 − 360px − 展开的左栏宽度）`）。
- 指针移动是帧合并的。指针释放持续一提交
