...
- Windows/Linux 在窗口中不呈现应用程序菜单。他们的无框
  ...
  右边缘。唯一的工作面板切换按钮始终固定在这些窗口控件之前，
  打开后仍保持原位并显示在工作面板标题上方；工作面板内容标题不含
  第二个折叠控件。目的地历史记录没有可见的 back/forward
...
- shell 启动时没有可见的工作面板。始终挂载的会话顶栏按钮与
  `Cmd/Ctrl + J` 都会切换活动会话的保留面板上下文，无需创建资源选项卡；
  折叠时保留资源，按钮在动画期间保持同一视口位置。没有活动会话或“设置”
  为活动页面时快捷键不执行操作。面板上下文触发器随后可创建浏览器或当前
  范围内的插件视图。
- 工件触发器自动创建或重用其资源，激活它，
...
- 每个资源都可从菜单关闭，活动资源还有直接标题关闭控件。关闭活动资源
  先选择右侧邻居，再选择左侧；关闭最后一个选项卡会隐藏面板。固定的会话
  顶栏按钮折叠面板但不删除选项卡，面板标题不渲染重复的折叠控件。
- 每个平台在打开、折叠和关闭最后资源时都请求原生预留宽度 0。
  BrowserWindow 边界保持不变，面板弹性分配使 MainChat 缩窄或恢复；左侧
  divider 调整面板宽度，原生窗口边缘正常调整窗口（D287、ADR 0148）。
...
- 10px 左边缘分隔符锚定到按下位置和起始面板宽度，然后跟随指针增量，
  不会跳动。宽度限制在 `244px–720px`。
- 指针移动按帧合并，释放仅持久化一次变化后的宽度；Escape、指针取消和
  丢失捕获都会回滚。ArrowLeft 扩宽、ArrowRight 缩窄，Home/End 到达边界，
  双击恢复 280px。
- 打开和关闭同时动画 dock 的 `width`、`flex-basis` 与有界
  opacity/transform；BrowserWindow 和固定顶栏按钮不移动，MainChat 连续
  重排而不会在第一帧前跳变。
- 打开、折叠和关闭最后资源始终请求原生预留宽度 0，因此操作系统窗口
  不扩展或移动 (ADR 0148)。重复目标是幂等的。
...
## 8a。输入框自动完成和剪贴板文件（D123–D125、D197、D262、ADR 0131）
