...
- 带有图标和简洁平行标签的紧凑导航目录，按以下顺序：
  ...
  2. **AI** — Lucide `Sparkles`（默认项、命令 Shell、提示词增强）
  3. **权限** — Lucide `Shield`（权限模式与审核）
  ...
  9. **子智能体** — Lucide `Bot`（内置与自建的并行工作智能体）
...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  的视觉分组：“偏好”（常规、AI、权限、快捷键）、“智能体”（指令、模型、技能、
...
- 审核主体、模型、思考强度和始终可见的自定义策略输入框位于此页。审核模型菜单可搜索；未固定模型时思考等级显示 `off` 且不可选，固定模型后显示 `off` 与该模型配置的等级。策略框初始为空，以简短占位文字说明留空使用默认策略；非空输入自动保存并替换内置策略，清空则自动恢复默认，无额外按钮。设置搜索将权限相关结果定位到这里。
