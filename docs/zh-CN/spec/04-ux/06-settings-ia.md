...
- 同一个“高级”折叠区承载该模型的上下文压力设置：Dynamic context 开关加阈值滑杆（40–95 %，读数给出占硬限制的份额与约等 token
  数）、Early compaction 开关（阈值、空闲秒数、静默开关）、Sleep-time digest 开关（每小时配额）。每个控件写入绑定并在下一次
  提示生效、无需重启；早于这些设置的绑定显示出厂默认值，取值按运行时同一边界钳制（ADR 0301）。
