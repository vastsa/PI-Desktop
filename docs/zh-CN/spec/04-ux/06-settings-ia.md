...
- **思考显示模式**使用菜单选择器，提供详细（默认）和紧凑。两种模式都使用一个
  ...
  搜索会索引该行和两种模式名称。
- **排队消息动效**使用菜单选择器，提供关闭（默认）、泡泡、柔光、水线四个选项。
  该设置只影响呈现：它在 Composer 中「即将发送」的排队消息行上叠加动效，
  让待发送的消息更显眼，行左侧的强调竖条本身保持不动。全局偏好以
  `queuedPromptAnimation` 持久化到宿主设置；缺失或无法识别的值使用关闭，即现有安装
  已持有的静态竖条。渲染层将解析后的值应用为根元素的 `data-queue-animation` 属性，
  更改无需重载即可生效；所有变体在 `prefers-reduced-motion` 下停止动画。设置搜索会
  索引该行、其说明和全部四个选项名称。动效装饰层裁剪在该行圆角边界内，静态强调条
  和行本身的阴影保持可见。
