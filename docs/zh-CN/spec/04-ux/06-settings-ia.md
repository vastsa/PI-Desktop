...
- **外观**卡：
  ...
  - **语言**：可搜索的选择行（不是卡片网格）。触发器显示当前语言的本地名称，或「跟随系统」加上检测到的本地名称。菜单把「跟随系统」钉在顶部并内嵌显示检测到的语言（例如「当前：简体中文」），然后按本地名称（永不翻译的 endonym）和英文名称列出每个已发布语言，供搜索和排序。选择更新 `settings.language`。新增语言只需加一份目录和一行注册表；选择器不硬编码选项列表。
...
- **提供商**工作室：
  ...
  - 新建对话框先只显示**服务**。命名端点按厂商平铺（OpenAI、Anthropic、Google、OpenRouter、DeepSeek、通义千问、月之暗面、智谱、硅基流动、火山方舟、MiniMax、小米、Kimi 编程等），不再分国际 / 国内。选中后显示服务 + API 密钥和主机摘要。自定义端点随后显示名称、Base URL，并在下一行并排显示 API 密钥与接口格式。名称（命名行）放在高级里。服务是可搜索锚定菜单（按显示名、厂商、主机过滤），不是原生下拉框。
