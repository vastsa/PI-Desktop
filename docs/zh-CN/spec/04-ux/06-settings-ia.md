...
设置 → MCP 在出站 MCP 工作台上方显示独立的「本机控制 Pi」卡片。
「启动时开启 MCP 控制」仅在保存的 `AppSettings.mcpControlEnabled` 严格为
`true` 时打开，默认关闭。说明文案解释本机其他程序可以通过本地访问令牌控制
Pi，并区分下方 Pi 连接的 MCP 服务器；修改后需退出并重新打开 Pi。
此开关代表下次启动偏好，不是实时服务状态；`PI_DESKTOP_MCP_CONTROL=1` 仍可
强制启用。保存期间禁用开关，失败时保留原值并显示错误提示。设置搜索收录
卡片标题、开关标签和说明。
