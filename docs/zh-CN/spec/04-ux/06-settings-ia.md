...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  ...
  MCP、子智能体）、“工作区”（导入、项目、索引）和“系统”（远程主机、信息）。标题使用柔和
...
- 单张健康卡展示 host 所有的工作区索引缓存：状态、已索引文件数、已索引体积、
  无法读取的文件数、最近更新时间
- 健康卡下方有一个独立的「代码库」小节，承载唯一的开关「索引加速 Grep」
  （`indexGrepBoost`，默认关）；它同时负责索引的生产与消费，
  因为 Grep 是它唯一的消费方。开启后，切换到不同工作区会把其索引标记为 `building`
  并在阻塞线程池中重建，`workspace.set` 保持快速；重新打开同一工作区至多每十分钟
  重扫一次，且重扫前先做一次只读 stat 比对，可见集无变化时完全跳过爬取。宿主自身
  的写入（Write / Edit / Bash）会立即让该 root 失效，快路径随即回退直到下次重建；
  可选的 `workspace-watch` 构建特性额外提供文件系统监视，覆盖宿主之外的编辑——失效
  只会损失速度，逐候选 stat 校验保证 Grep 的结果始终与回退路径一致。健康卡在构建期间每秒轮询
  一次 `index.status`，窗口隐藏时暂停；区分大小写的字面量 Grep 查询此时可由索引服务，
  其余查询照常全量走查。文案不承诺结果变化，只说明符合条件的查询会更快。再加一个
  「索引新文件夹」开关要么与它重复，要么建出无人读取的索引，因此该小节只保留一个
- 操作为「重建索引」与「清理索引」。重建把当前工作区扫描进可丢弃缓存；清理删除
  其 root namespace。两者都调用 host 生命周期 RPC 并用返回状态刷新卡片。重建只在
  开关打开时提供——开关不喂它的话，索引就只是白扫一遍盘、白占一份盘；清理始终可用，
  好让遗留索引仍能被删掉
- 文案说明索引是可重建的本地缓存，数据不会离开本机，且 Grep 结果不依赖它
- 空态：当前工作区还没有 root，只提供「建立索引」动作，并受同一条开关门控——空态不会
  提供一个开关会让它无人读取的建立动作。加载失败显示重试而不是空白卡片
...
2. 导轨顶部显示搜索药丸，底部固定返回应用程序操作并与主侧边栏底部图标行同一条线，
   ...
   子智能体、导入、项目、索引和信息（开发者模式开启时，远程主机位于索引与信息之间），
