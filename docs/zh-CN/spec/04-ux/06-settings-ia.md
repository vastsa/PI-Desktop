...
- 带有图标和简洁平行标签的紧凑导航目录，按以下顺序：
  ...
  3. **用量** — Lucide `Activity`（已完成回合的 token 合计）
  4. **快捷键** — Lucide `Keyboard`（键盘快捷键）
  5. **指令** — Lucide `FileText`（全局和项目指令文件）
  6. **模型** — Lucide `Bot`（提供商和默认模型）
  7. **技能** — Lucide `BookOpen`（可复用的智能体指令）
  8. **MCP** — Lucide `Server`（智能体连接）
  9. **子智能体** — Lucide `Bot`（用于并行工作的个人智能体）
  10. **导入** — Lucide `Download`（从其他工具引入会话）
...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  的视觉分组：“偏好”（常规、AI、用量、快捷键）、“智能体”（指令、模型、技能、
...
2. Rail 显示搜索+返回和精确的常规、AI、用量、快捷键、指令、模型、技能、MCP、
