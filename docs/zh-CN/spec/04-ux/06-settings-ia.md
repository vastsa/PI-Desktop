...
- **提示词增强**卡控制输入框的「增强提示词」操作（ADR 0121）。增强模型行在未指定
  提供商/模型前跟随输入框当前模型。两个模板编辑器在未保存覆盖值时显示内置默认
  文本，因此屏幕上显示的值就是实际生效的值；卡片级恢复操作会一次清空两项覆盖。
  用户模板编辑器提供插入草稿变量的操作，并拒绝会让模板缺少该变量的保存。设置
  搜索会索引该卡与两个模板行。
