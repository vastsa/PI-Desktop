...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  的视觉分组：“偏好”（常规、AI、快捷键、语音输入）、“智能体”（指令、模型、技能、
...
本地语音输入是独立的、可选的设置目的地，由 `AppSettings.voice` 支持，默认关闭。
它使用桌面主进程和本地下载的 Whisper/SenseVoice 模型，不配置宿主语音 provider。
页面提供麦克风权限、输入设备、语言列表、中文输出变体，以及模型下载/删除操作。
只有 `voice.enabled` 为 `true` 时 Composer 才显示麦克风；浮层显示录音和转写状态，
成功的文本插入当前草稿。
...
2. 导轨顶部显示搜索药丸，底部固定返回应用程序操作并与主侧边栏底部图标行同一条线，
   ...
   子智能体、导入、项目、语音输入和信息（开发者模式开启时，云同步与远程主机依次位于项目与语音输入之间），
...
6. 常规仅显示主机支持的外观卡；人工智能
   ...
   目的地显示键盘快捷键卡；语音输入目的地显示本地麦克风和模型控制；信息显示开发者卡。
   不呈现其他设置目的地
7. 提供商机密从不显示原始密钥值
