...
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
| 工作面板宽度（打开） | `244px–720px`（新用户默认360px），固定为承诺宽度 | 组合的创建触发器保持内容的完整面板宽度；已保存宽度不变；该面板是流入列，从不扩展操作系统窗口（D154/D163、ADR 0033） |
| Composer shell 最小 | 〜80像素 | 一行草稿+工具栏填充 |
| Composer 工具栏 | MainChat `≥515px` | 左右控制组保持一行且不收缩；模式/权限标签保持单行并省略截断 |
