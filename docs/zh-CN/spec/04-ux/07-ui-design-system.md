...
| 主窗格最小可读宽度 | 360像素 | MainChat 硬下限；被突破前先让展开的左栏让位（ADR 0238） |
| 工作面板宽度（闭合） | 0像素 | 默认隐藏 |
| 工作面板宽度（打开） | `≥244px`（新用户默认360px），并受 `客户端宽度 − 360px − 展开的左栏宽度` 限制（无固定像素上限） | 面板是流入列，宽度取自现有客户区；分隔线由渲染器拥有（ADR 0033 / ADR 0151 / ADR 0238）；已保存宽度不变 |
| Composer shell 最小 | 〜80像素 | 一行草稿+工具栏填充 |
| Composer 工具栏 | MainChat `≥360px` | 左右控制组保持一行且不收缩；模式/权限标签保持单行并省略截断 |
