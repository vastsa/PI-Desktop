...
- 每个控件都可以通过 Tab 键盘访问
- 当 `AppSettings.voice.enabled` 为 `true` 时，输入框渲染可选的本地语音输入麦克风和录音/转写浮层。
  成功的本地结果插入当前草稿；宿主语音能力仍只能由 IPC 与插件调用（ADR 0291）。
...
- Main 把图片字节存在 `attachments/<sha256>`，仅当所选 models.dev 模型接受
  ...
  代理用文件工具跟随路径。
- 本地语音输入是可选功能，默认关闭。启用后，输入框麦克风启动主进程录音生命周期，
  提供取消和有限的录音/转写反馈，并把最终文本插入草稿。它与宿主 `speech/*` 绑定分开；
  详见 `03-runtime/23-voice-input.md`。
