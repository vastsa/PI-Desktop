...
- 触发器：file/URL 引用和 browser_preview 在原始会话运行时上下文中
  创建或激活资源选项卡。browser_preview 事件携带 `sessionId`，渲染器保留
...
- 可见标签是自然语言操作（`read`、`Ran`、`Searched`），
  不是原始函数名称。首字母大写形式是由规范工具名推导得到的展示标签（`read` 显示为 `Read`），
  本身不是身份：模型调用的是小写规范名。运行中的操作采用进行时形式。
- 主要参数是固定的单行等宽提示。
...
| 工具 | 积木 |
|---|---|
| `read` | `File content` — 从文件扩展名中突出显示的语法 |
| `write` | `Written content` — 从目标扩展中突出显示 |
| `edit` | `Changes` — 紧凑差异，仅当没有 ReviewChangeCard 拥有时 |
| `bash` | `Command`（外壳）、`Output`、`Errors`（错误色调）；空通道被省略 |
| `glob` | `Files` — 可点击的工作空间路径 |
| `grep` | `Matches` — 按文件分组，带有 `line` 装订线和 `outputMode: content` 的可点击路径标题； `filesWithMatches` 的可点击路径列表； `path` → `count` 的点击计数字段 |
...
生命周期行（`task_wait`/`task_list`/`task_stop`）仍是紧凑工具行 —— 它们不是拓扑
...
- **摘要是它所报告的名册**，按子智能体名称，读自 `details.delegations[]`
  （`task_wait`/`task_list`）或 `details.stopped[]`（`task_stop`）。重复出现的
...
- 每个节点显示定义名称、简短描述、明确结果、
  ...
  `running` 节点重建为 `aborted`；`task_stop` 的 `details.stopped[]` 也是状态来源，
