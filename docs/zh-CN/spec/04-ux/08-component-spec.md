...
- 在工作面板中打开 HTTP(S) 链接会新增浏览器资源标签，不覆盖原网址。
  ...
  不承诺自动休眠或应用重启后的 DOM 恢复。
  地址栏和普通页内导航留在当前标签。网页请求打开新窗口时，遵循链接打开位置设置，
  新增工作面板标签或打开系统浏览器。

- BrowserPreview 先创建目标资源标签，再开始导航，不改写原标签。后台导航保存到该会话
  上次选择的标签；尚无标签时，由第一个标签消费一次。新空白标签返回空状态。
...
| 新会话（推理模型） | 模型 × 推理芯片显示模型及其绑定的默认思考等级；未匹配模型从关闭开始但保留手动阶梯 | 用户可以选择绑定已启用的任何级别，未匹配模型可以选择任意规范级别 |
...
- 目录命中的继承默认模型支持推理时，新会话以该模型存储的默认思考等级启用，
  并钳位到已启用档；绑定没有默认值时才回落到最高已启用档。未匹配模型没有
  显式绑定默认值时从 `off` 开始，但 Composer 仍保留完整规范阶梯供手动启用。
  重新打开或重复使用现有会话保留其持久选择。
