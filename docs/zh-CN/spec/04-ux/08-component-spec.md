...
| 多个工件 | 标题栏保留可横向滚动的标签条；固定 `+` 菜单列出 Review、满足能力条件的远程 Terminal 和所有当前范围内的插件视图，不重复已打开的资源标签。 |
| 会话切换 | 目标会话保留的打开状态、选项卡、活动选项卡和浏览器资源自动替换前一个会话的面板上下文；上下文都不会被删除。远程 Terminal 始终绑定其远程会话，不会被本地会话复用。 |
...
| New 启动器激活 | 主体承载简洁的 Review、符合条件的远程 Terminal 与插件视图按钮；每行会用其目标替换启动器标签页，或激活已存在的单例视图；该页面可独立关闭 |
...
- 回顾事实：host-core 为每个 `details.review` 添加一条有界 `details.review` 记录
  ...
  后工具哈希和
返回冲突而不覆盖以后的工作。
- Terminal 的边界与权限：首期工作面板 Terminal 仅用于远程会话。只有 Host 声明支持 Terminal 时才显示入口；Host 只允许经 SSH 配对的 owner 设备打开和操作终端。Shell 在远端 `pi-host` 上以其操作系统用户权限运行，并以远程会话根目录为工作目录；该目录不是文件系统沙箱。界面会标明 Host 和会话。本地桌面会话仍不提供交互式 Terminal：Agent Bash 输出继续显示在 transcript 中，不会变成本地 shell；插件也不会获得 PTY API。
- Terminal 生命周期：打开 Terminal 会在远程 Host 创建 PTY，或重新连接到绑定该会话和 owner 的既有 PTY。折叠面板、切换会话或传输断开时，UI 取消输出订阅，远端进程仍可重新连接。显式关闭 Terminal 标签会关闭其 PTY。重新连接时显示 Host 有界输出回放；输入不会回放，因此重连不会重复提交命令。Host 的能力与授权校验始终是最终依据；渲染器的显示条件只是易用性控制。
- 标题栏标签：标签条是包含所有打开的 Review、文件、插件视图或符合条件的远程 Terminal 的 `tablist`。
