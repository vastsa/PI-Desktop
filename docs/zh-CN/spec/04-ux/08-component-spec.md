...
### 11.8 斜线命令、@ 文件引用和剪贴板文件（D123–D125、D197、D209、D262、D362、ADR 0024、ADR 0059、ADR 0070、ADR 0131、ADR 0308）
...
- 斜线模式（`/` 在位置 0 处输入，光标位于第一个标记内，无
  ...
  匹配的字符以重音突出显示。
- 在智能体模式下，`@` 菜单还会在文件行之上以"智能体"分组列出可委派的
  智能体（ADR 0308）。两个分组各自带有标题 —— “智能体”与“文件” —— 因此
  两类内容不会被读成同一区块；文件行不会作为无标题的尾部留在智能体标题之下。
  智能体行使用与子代理设置页、模型选择器一致的机器人徽标，文件行保留自身图标，
  图标因此强化了分组。接受其中一个会生成内联 chip —— 与已完成的文件引用相同的
  原子 chip —— 并带上机器人徽标：委派对象是用户选中的**一个**事物，因此它像一
  个整体那样显示和删除，而不是可被单个按键删掉一半的 `@name` 文本。该 chip 在
  发给模型前会还原为 `@name`；当其前方有文本时会补一个前导空格，因为发送时的
  解析器只在开头或空白之后读取 `@token`。重启后恢复草稿时，委派对象仍还原为委派
  对象，绝不会变成文件 chip；委派对象也永远不会变成附件。在转录中，同样的 token 会
  渲染为逐 mention 的委派 chip —— 与文件引用相同的 `.composer-chip` 表面，带机器人
  徽标，且不带按钮态，因为委派对象指向的是一次运行，而不是可打开的东西。这些偏移量
  在消息发出时记录，因此委派对象被删除后，先前消息上的 chip 依然保留。chip 显示
  的是裸标识（`explorer`），而非用户输入的 `@token`：徽标本身已经表明这是委派对象。
  输入框内的 chip 与 `@` 菜单行仍保留 `@`，因为那正是用户正在选择或输入的 token；
  无论 chip 如何渲染，存储的偏移量始终索引原始输入形式。若存储的
  mention 与文本对不上，则回退为渲染整段原始草稿。接受其中一个会插入
  `@name `，用户可在同一草稿中继续
  撰写任务描述。发送时，main 会将草稿改写为一条显式的 `Task` 指令加上用户
  自己的正文，而转录仍以原始 `@agent …` 草稿作为调用 chip 保留。只有当
  token 命中委派目录**且**工作区中不存在同名文件时，它才被视为智能体；同名
  文件优先。不在目录中的名称保持普通文本。Plan 与 Goal 模式不显示该分组，
  从这两种模式手输并发送 `@agent` 会被明确拒绝并弹出提示，而不是当作普通
  文本发送，且草稿保留以便切换模式后重试。
...
结构化撤消快照，而回合仍未得到答复。智能停止
  ...
  不得恢复或复制已提交的草稿。当不存在快照时 —— 编辑重发不会记录快照 ——
  恢复逻辑读取该行的 `command`，即被改写轮次中用户输入的原文；仅对从未被改写
  的普通提示才回退到 `content`。若恢复被改写后的 `content`，等于把一段模型指令
  放进输入框，等用户再次发送。
