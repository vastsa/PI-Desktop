...
**崩溃上报必须带上诊断。** 崩溃路径会报出宿主进程的**退出码**：Windows 上的硬故障（`0xC0000005` 一类，Electron 以负的有
符号整数交付）与插件自己调用 `process.exit(1)` 是两类完全不同的问题。退出码会出现在加载错误、`failed` 服务状态、`plugin.crash`
审计记录与 `plugin` 日志通道里。插件 stdout/stderr 可能包含工作区数据或敏感信息，因此不会复制进崩溃上报。不新增任何持久化，也不涉及
新的权限或 API 面。
...
- 错误代码？
- 尝试？ / 延迟女士？ （服务重新启动）
- exitCode?（崩溃）、exitCodeHex?（Windows 硬故障）
