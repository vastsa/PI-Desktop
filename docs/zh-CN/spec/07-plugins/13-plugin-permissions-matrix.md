...
| `agent.permission.deny` | 中等 | `contributes.permissionDeny` 并入宿主 deny-first 叠加 | 安装时确认 | 只能 deny；不能删除用户规则或增加 allow。空 `{}` 仍需要该权限。不在 `HIGH_RISK_PERMISSIONS` 中（D420 / ADR 0249） |
