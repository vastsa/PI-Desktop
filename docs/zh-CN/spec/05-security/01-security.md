...
- 默认关闭。仅当保存的 `AppSettings.mcpControlEnabled` 严格为 `true`，或设置了
  `PI_DESKTOP_MCP_CONTROL=1` 时启动。设置 → MCP 中的本机控制开关独立于出站
  MCP 客户端，修改后需退出并重新打开应用。缺失、读取失败或非布尔值不会启用
  服务；环境变量 `1` 强制启用，其他值不覆盖保存的偏好。
