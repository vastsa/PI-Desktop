...
- **前提条件**：有来自两个构建的 Windows x64 便携包（A、B），以及对应的 NSIS
  安装程序和 `latest.yml`；有干净用户配置；账户是无需管理员提升的标准用户。
- **步骤**：1) 检查发布目录和 `latest.yml`。2) 不运行 NSIS 安装程序，直接启动便携包 A。
  3) 确认进程环境包含 `PORTABLE_EXECUTABLE_FILE`，解压后的应用路径为
  `%TEMP%\PI-Desktop-Portable\PI-Desktop.exe`。4) 将运行中的应用固定到任务栏并记录固定目标和图标。
  5) 退出 A 后再启动其它程序；确认启动器删除解压目录，并记录停止期间固定项是否消失或显示空白。
  6) 再次启动 A，确认同一路径被重新创建、运行中图标正确且固定目标恢复。7) A 退出后启动包 B，
  确认它使用同一个固定路径。8) 在隔离运行中让 A 仍运行时启动 B，记录共享目录被删除/覆盖的风险；终止两者并将
  同时启动便携启动器标记为不支持。9) 调用检查更新。10) 确认设置 → 信息提供发布页而不是“重启以更新”。
- **预期**：两个 Windows 工件都无空格并已上传。`latest.yml` 只指向 NSIS 安装程序。
  每个便携包无需安装向导或管理员提示即可启动，使用现有应用数据目录，并报告更新模式 `manual`。
  可用更新不会下载或运行 `PI-Desktop-Setup-<version>.exe`。显式解压路径在不同构建和多次启动间保持稳定，
  运行中的任务栏分组与图标查找使用同一路径，而不是新的构建 ksuid。启动器会在解压前和退出后删除该路径，
  因此停止期间固定项可能不可用或为空白；重新启动会重建路径并恢复运行中图标/固定目标。同一用户的第二个便携
  启动器不得并发运行，否则可能删除或覆盖第一个启动器的共享解压目录。再次启动从同一配置恢复会话。
- **链接规格**：`01-product/01-product-scope.md`、
...
- **状态**：打包契约由单元测试覆盖；`Windows portable E2E` 工作流自动执行
  原生打包、解压、清理、固定、重新启动、跨构建和并发 smoke；原生任务栏资格
  验证仍需具备桌面会话的 Windows 运行器。
