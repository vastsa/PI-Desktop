...
- **先决条件**：项目绑定的 Agent 会话使用确定性
  发出部分辅助流的提供商装置，终止一次，
  然后下一个请求成功；第二场比赛可以终止四次；
  第三个装置在一次尝试中于标头之前返回
  `OpenAI API error (502)`，并在下一次尝试中于流中返回它；
  第四个装置返回连续五个 502；第五个装置返回带
