...
#### E2E-011c：按会话保留 Composer 草稿

- **先决条件**：已配置提供商；会话 A 和 B 存在且空闲。
- **步骤**：1）在 A 输入但不发送。2）切换到 B，确认为空，再输入不同文本和
  文件引用。3）返回 A。4）依次进入插件、Pull requests、Scheduled 和设置并
  返回聊天；跨越空态/对话态并切换项目后再次返回 A/B。5）创建新会话并删除
  B。6）重载 renderer 或重启应用。
- **预期**：A/B 各自恢复文本和文件引用，新会话不继承任何草稿。页面、项目及
  空态/对话态切换不会丢失未发送内容；删除 B 会清理其缓存，renderer 重载或
  应用重启会清除全部草稿。异步发送完成时只清除提交来源会话，不影响当前会话。
- **链接规格**：`04-ux/08-component-spec.md`、`04-ux/09-interaction-patterns.md`、
  ADR 0149、D288
- **验收**：C（会话隔离和输入）
- **里程碑**：M2
- **状态**：源级回归覆盖（`composer-draft-cache.test.mjs`）；完整 UI 场景 Draft

#### E2E-011h：会话内显示附件图片与本地 Markdown 图片

- **先决条件**：已配置支持视觉的提供商；一个会话的工作区包含图片文件。
- **步骤**：1）在 Composer 粘贴一张图片并带短提示发送；再粘贴第二张图片。
  2）检查用户消息气泡。3）重载会话/应用后重新打开同一会话再检查。4）让
  助手用 Markdown（`![alt](path)`）引用本地图片，并单独确认远程 `https://`
  图片仍正常渲染。5）让 Markdown 引用一个不存在的路径。
- **预期**：粘贴/上传的图片附件通过受根校验的 `fs/readImageDataUrl` 通道在
  用户气泡中显示为有界内联缩略图，位于提示文本上方；多张图片各自成缩略图。
  重载并重新打开会话后，图片仍从持久化 ref 恢复显示。助手回复中的本地
  Markdown 图片内联渲染，点击在工作面板文件查看器中打开；远程图片仍直接
  渲染。引用不存在的路径时回退为文件查看器 chip，绝不显示损坏图片或读取
  任意文件。
- **链接规格**：`03-runtime/01-ipc-protocol.md`（§fs）、
  `04-ux/08-component-spec.md`（§8）
- **验收**：C（聊天和直播）、F（持久化）
- **里程碑**：M2
- **状态**：源级回归覆盖（`message-image-display.test.mjs`）；完整 UI 场景 Draft
...
- **步骤**： 1) 打开聊天路径。 2) 检查顶部的 46px 栏
  交谈区。 3) 确认其显示简洁的 session/task 标题和新任务、搜索、固定的
  工作面板动作按钮；确认
...
- **先决条件**：应用正在运行且存在活动会话。
- **步骤**：1）重启并确认面板默认关闭，会话顶栏右上角显示唯一的面板
  切换按钮；记录 BrowserWindow、MainChat、按钮和面板边界。2）单击按钮
  打开和关闭，再用 Cmd/Ctrl+J 重复，并在动画过程中采样边界。3）打开文件、
  Browser、Review 和范围内插件资源，操作统一菜单并关闭最后资源。4）在会话
  A/B 之间切换并生成后台工件。5）双向拖动左 divider 并越过 244px/720px，
  用 Escape、指针取消和释放分别结束；双击并测试 Arrow/Shift+Arrow/Home/End。
  6）在 macOS 与无框 Windows/Linux 上重复，包括最大化、全屏和最小窗口。
  7）注入失败及过期的零 reservation 响应后重试。8）重启。
- **预期**：同一个会话顶栏按钮在关闭、进入、打开和退出状态下保持挂载并位于
  相同视口坐标。Windows/Linux 上它位于 minimize/maximize/close 之前；面板
  打开时按钮显示在面板标题之上，资源关闭按钮仍可点击，且面板标题没有重复的
  折叠控件。按钮在 DOM 绘制顺序上位于面板之后，显式建立非拖拽指针命中区，
  面板可见期间持续显示 active 高亮；面板标题的拖拽区域不能夺走 hover 或 click。
  按钮发布按下状态，并与 Cmd/Ctrl+J 共用活动会话的 toggle 路径。
  ...
  MainChat 同步缩窄，不被覆盖且第一帧前不跳变。关闭先分离原生 Browser/插件
  表面，再向右滑出并把 width/flex-basis 收回到零，MainChat 恢复原宽；动画结束
  或超时保护后才卸载。Windows 不出现白闪或旧预览帧，失败/过期响应不能提交
  旧 presentation。

  资源去重、邻居选择、菜单焦点、会话上下文及后台隔离规则保持不变。固定按钮
  折叠面板时保留 tabs；关闭最后资源隐藏面板；重启只恢复首选 `{width}`。

  divider 按下时不跳动，向左拖动扩宽、向右拖动缩窄，renderer 预览按帧合并，
  释放只持久化一次变化后的有界宽度。Escape、取消、丢失捕获和卸载恢复按下时
  宽度；双击恢复 280px。ArrowLeft 扩宽、ArrowRight 缩窄，Shift 步长 32px，
  Home/End 到达 244px/720px。原生窗口边缘正常调整 BrowserWindow，不因面板
  可见而改写其首选宽度。原 ContextPanel overlay 不存在。
- **链接规格**：`03-runtime/01-ipc-protocol.md`、`04-ux/01-ui-ia.md`、
  `04-ux/07-ui-design-system.md`、`04-ux/08-component-spec.md`、
  `04-ux/09-interaction-patterns.md`、ADR 0148、D142、D207、D287
- **验收**：F（持久化）、质量
- **里程碑**：M5
- **状态**：单元覆盖（`work-panel-resize.test.mjs`、
  `work-panel-window.test.mjs`、`work-panel-presentation.test.mjs`、
  `work-panel.test.mjs`）；完整 UI 场景仍为 Draft
...
窗口内部和左侧导航占用了回收的空间
  ...
  从主聊天、设置和打开的工作面板。工作面板打开时，确认 AppShell 面板切换
  按钮与原生控件带都固定在视口右侧，面板标题为两者预留空间且资源关闭按钮
  仍可点击。在
...
PI-Desktop 图标；两个表面都不会暴露库存 Electron 名称或图标。
  macOS 遵循本机菜单约定和加速器。
  Windows/Linux 窗口内不显示应用程序菜单；导航和右侧控件不会与拖动区域
  冲突，键盘快捷键保持运行。唯一的工作面板切换固定在原生控件带之前，打开
  后面板标题不含重复折叠按钮，并为两个固定区域预留空间。检查更新
  从 macOS 系统菜单和
  设置表面并显示结果的最新状态。更换窗
  命令等待渲染器准备就绪，无需
...
- **步骤**：
  ...
  3. 把窗口拖到横跨两块显示器边界的位置并松开，确认它完整落在某一块显示器的
     工作区域内，且尺寸不变。
  4. 面板打开时重复跨显示器拖动，确认窗口尺寸不变、面板保留已提交的内部宽度，
     且目标显示器上的 reservation 仍为零。
  5. 把窗口拖回第一块显示器，确认不会发生面板驱动的尺寸或位置调整。
...
- **预期**：每次松开鼠标都让窗口停在用户放下的显示器与位置上。横跨边界的放置会被
  ...
  而不是最初那一块。移除窗口所在的显示器仍会把它迁移到在线显示器；重新连接
  不会增加面板 reservation 或代表内部面板移动窗口。
- **链接规格**：`03-runtime/01-ipc-protocol.md`、
...
- **步骤**：
  ...
  3. 工作面板打开时拖动外部右边缘，确认 BrowserWindow 正常调整而面板首选
     宽度不变；再拖动内部 divider，确认仅面板宽度改变且 reservation 为零。
  4. 等待调整大小稳定后关闭并重新启动应用。
- **预期**：无边框外壳仍提供原生边缘和角落命中区域，最小尺寸保持
  1040×700，恢复看门狗不会与慢速调整大小流竞争。所有原生边缘正常调整
  BrowserWindow；内部 divider 独占 244–720px 面板宽度，MainChat 反向重排。
  最后稳定的窗口边界会在重新启动后恢复，没有面板驱动的宽度或 x 偏移。
- **链接规格**：`03-runtime/01-ipc-protocol.md`、`04-ux/01-ui-ia.md`、
  `04-ux/07-ui-design-system.md`、`04-ux/08-component-spec.md`、
  `04-ux/09-interaction-patterns.md`、ADR 0148
- **验收**：A（应用外壳）、F（坚持）、质量
- **里程碑**：M6+
