...
- **预期**：可见 shell 标识为 `PI-Desktop`；空荡荡的家英雄
  渲染 100px `HomeMascotLogo` GIF，首帧短暂停留后循环挥手；
  指针悬停不改变节奏或几何形状，减少运动时显示静止首帧。
  expanded/collapsed
...
- **步骤**：
  ...
  2. 检查侧边栏和启动画面中渲染的 `BrandLogo` 源，
     并检查空首页英雄中的八帧 `HomeMascotLogo` GIF。
     将鼠标悬停在吉祥物上并验证节奏不变。
...
- 空首页英雄渲染 100 像素的八帧吉祥物 GIF，首帧短暂停留后
    循环挥手。指针悬停不改变节奏；减少运动时静止首帧
    仍然可见。
