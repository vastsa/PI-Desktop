...
- **步骤**：1) 打开“设置 → 智能体 → 子智能体”，点击“新建子智能体”，确认表单上方有
  “从模板开始”区域，包含 Explorer、代码审查员、测试执行者、修复者和空白开始五个选项。
  2) 选择 Explorer，确认名称、描述、Read / Glob / Grep / Bash 工具、60 轮上限和完整提示词
  均已填入，模型仍沿用当前会话。3) 重新打开并选择修复者，确认工具、80 轮上限和提示词更新，
  且显示可修改工具的提示。4) 打开模型选择器，确认每个提供商中标记为
  `availableForSubagents` 的模型都出现，而关闭标记的模型不出现。选择一个模型，确认保存值为
  `vendorKey-or-name/modelId`。5) 选择“自定义（provider/model）…”，输入一个模型 pin 并保存；
  再选择“沿用当前会话的模型”，确认模型值为空。6) 禁用所有委派模型，确认回退为带有模型设置提示的
...
#### E2E-196b：未签名的 macOS 软件包包含打开说明

- **先决条件**：默认未签名的 macOS 发布已为至少一个本机架构生成 DMG 和 ZIP 工件。
- **步骤**：1) 打开 DMG 并检查根目录。2) 不解压应用内容，检查 ZIP 根目录。3) 阅读
  `PI-Desktop-macOS-opening-help.txt`。
- **预期**：两个软件包的根目录都包含同一份打开说明文件，其中包含
  `xattr -cr /Applications/PI-Desktop.app`，并说明该命令仅适用于 macOS 报告可信的
  未签名应用已损坏时的场景；已签名/公证版本无需执行。说明不会声称未签名工件已通过
  Gatekeeper 资质验证。
...
| B——模型配置 | E2E-005、E2E-005G、E2E-006、E2E-007、E2E-038、E2E-050、E2E-052、E2E-055、E2E-066、E2E-080、E2E-082、E2E-151、E2E-005J、E2E-199 |
...
| M6+ | E2E-121、E2E-122、E2E-123、E2E-142、E2E-148、E2E-150、E2E-151、E2E-168、E2E-199、E2E-200 |
