...
- **预期**：有模型 id 时答案下方显示模型徽章。有用量后，紧凑上下文
  ...
  非默认 Advanced 覆盖仍然生效。检查器内的生成速度仍是已完成回合快照；回合进行中时流健康条另有实时 `tok/s`（D428）。重试重新
...
- **前提**：已配置会流式输出答案令牌的提供商会话（或按已知节拍发出文本增量的测试替身）。
- **步骤**：
  1. 发送一条会持续数秒流式作答的提示。
  2. 在答案文本出现前、工作中 / 运行活动指示器可见时，观察状态条。
  3. 答案令牌开始流式输出后，确认活动条隐藏，并保留紧凑的流式速率芯片。
  4. 可选：暂停或节流上游流足够久，让滑动窗口排空，然后再恢复。
  5. 让回合结束，确认实时速率清除；打开上下文用量检查器，确认已完成回合的生成速度仍显示。
- **预期**：
  - 实时速率以 `N tok/s`（或对应本地化文案）出现在流健康指示器旁，并在回合中更新。
  - 流中若尚无提供商输出用量，读数标为约数（`≈` / 「约」）。
  - 停顿会把实时速率降向 `0 tok/s`，但在回合仍活动时不清除芯片。
  - 空闲清除实时芯片；检查器生成速度仍是已完成回合快照（D428）。
- **链接规格**：`04-ux/08-component-spec.md` §8.3、
  `04-ux/09-interaction-patterns.md` §3.2a、D428
- **验收**：C（对话和直播）
- **里程碑**：M6
- **状态**：窗口速率数学已有单元覆盖；UI 旅程 Draft
