...
- **步骤**：1) 确认插件页列出 `pi.browser`，已启用，不可卸载。
  ...
  文件（`browser_preview`），再通过 tool_search `cdp` / `Browser` 做 snapshot。
...
- **预期**：启动项没有宿主 Browser 行。预览打开插件视图并实时重载文件。
  插件工具 `plugin_pi_browser_Browser` 在 tool_search 后可以 snapshot。Plan 拒绝
...
- **预期**：写入、编辑和插件工具不可见且不直接
  ...
  `PLUGIN_DISABLED_IN_PLAN`；这些工具不会更改任何文件。`bash` 运行
...
- **前提条件**：本地开发服务器正在运行；存在 URL 或 browser_preview 工件。
...
- **步骤**：1) 开始轮流 A 和 B，然后返回 A。 2) 让 B 发出流式传输
  ...
  当 A 可见时，browser_preview 伪影。切换回每个会话。
...
即使 A 的较旧负载稍后完成，也会在 B 上执行。仅明确通知或
  ...
  A 中 B 中没有瞬态 open/close 闪存； B 的 browser_preview 携带 B 的
...
- **预期**：
  ...
  - 在同一活动回合内，内容未变化且不含委派的活动组，不因文本更新重建了
    回合级委派 Map 而再次渲染。工具内容变化仍会渲染；后续 task_wait 结果
...
#### E2E-106：`submit_plan` 拒绝进入可编辑计划并重新提交新工件
...
#### E2E-142：后台委托通过 `task_wait` 收敛并遵守权限作用域
...
- **验收**：B（agent）、安全、质量
- **里程碑**：MVP 后（R7 v1）
- **状态**：部分自动化（`pnpm test:e2e:trusted-extensions`）；Agent 模式工具调度、tool_search 延迟、hooks、阻止和结果替换已覆盖；Plan 模式门控与核心工具冲突仍需额外验证
