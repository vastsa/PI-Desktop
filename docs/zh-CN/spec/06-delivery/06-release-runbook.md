...
数据仍在现有应用数据目录。便携版请求 user 执行级别，因此启动不需要管理员权限。自解压应用运行时使用固定路径
`%TEMP%\PI-Desktop-Portable\PI-Desktop.exe`，使不同构建和多次启动的运行中身份保持稳定。启动器会在解压前和退出后
删除该目录；因此指向解压文件的任务栏固定项在应用停止期间可能消失或显示空白，重新启动后才会恢复。相同用户的所有
便携启动器（包括不同版本）共享该路径，不得并发运行；已安装的 NSIS 副本使用独立目录。
