...
These three modes are unchanged (D420 / ADR 0249). Always-deny is an overlay,
...
- Deny-first overlay (D420 / ADR 0249) wins over `auto`, session grants,
...
Per-call host evaluation order (frozen, D115 / D198 / D420):
...
### Deny-first overlay (D420 / ADR 0249)
...
- MCP tools
- tool group toggles
- command allowlist (command denylist shipped as `permissionDeny.commands`, D420)
- dry-run mode
