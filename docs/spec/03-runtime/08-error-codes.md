...
| `INDEX_UNAVAILABLE` | no | the index store could not be opened or the rebuild worker failed; Grep keeps working via rg |
| `INDEX_ROOT_OUTSIDE_WORKSPACE` | no | index.status/rebuild/clear was called with a root that is not the active workspace root |
| `INDEX_REBUILD_FAILED` | no | a background index rebuild failed partway; the index is marked failed and Grep keeps working via rg |
