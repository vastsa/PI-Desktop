...
- Nothing is rewritten on disk and no write path changes: `providers.update` still persists exactly the bindings the client sends, so a save issued from a degraded view replaces the stored array as it always did. See `03-runtime/12-provider-config-schema.md` §2.
