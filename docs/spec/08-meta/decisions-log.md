...
| D420 | Deny-first permission rules | **Amend D115: keep `ask` / `accept-edits` / `auto`. Overlay `AppSettings.permissionDeny` ∪ enabled plugins granted `agent.permission.deny` whose activation scope matches the session workspace. Schema `{ tools, paths, commands }` globs; unknown keys rejected; 256 entries × 512 chars; empty `{}` is valid. A match is `PermissionDecision::Deny` after Plan/Goal hard deny and before external-path / low-risk / auto / grants. Plugins can only add rules. Hits reuse `TOOL_DENIED`. See ADR 0249.** | `auto` was an unbounded YOLO equivalent. Users and policy plugins need an always-deny list that cannot be loosened by a plugin or a session grant. |
...
## 2026-09-15 — Deny-first overlay, not a fourth mode (D420)

- Permission modes stay `ask` / `accept-edits` / `auto`. `auto` remains the
  YOLO equivalent for high-risk tools, but it is no longer unbounded: a
  user-owned `AppSettings.permissionDeny` and granted plugin contributions
  can name tools, paths, and command prefixes that stay denied.
- The overlay is the union of the user's JSON and `contributes.permissionDeny`
  from enabled plugins that hold `agent.permission.deny` and whose activation
  scope matches the session workspace. Plugins are deny-only; there is no
  `allow` key, and a plugin cannot delete a user rule.
- Evaluation order is frozen: Plan/Goal hard deny, then deny-first, then
  outside-path / low-risk / auto / grants / the confirmation card. A hit is
  the existing `TOOL_DENIED` code so the model is not taught which glob
  fired.
- Settings → AI → Permissions exposes the JSON under the mode select.
  Schema: `{ tools, paths, commands }`, unknown keys rejected, 256 × 512.
  The SDK validates shape; host-core compiles globs with `globset`.
- See ADR 0249.
