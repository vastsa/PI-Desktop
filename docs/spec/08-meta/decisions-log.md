...
- Decision D364 amends D120 / D126 / ADR 0022: the Windows x64 lane publishes
  ...
  `unpackDirName` to `PI-Desktop-Portable`, so the running executable resolves
  to `%TEMP%\PI-Desktop-Portable\PI-Desktop.exe` across builds and launches.
  The launcher deletes that directory before extraction and after exit, so a
  stopped pin may be blank until relaunch. All portable wrappers for one user,
  including different versions, share the directory and must not run
  concurrently.
