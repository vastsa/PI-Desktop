...
- Recording happens only on an accepted submission. The four accept paths — a
  ...
  home composer has no conversation, so it recalls nothing, and a prompt sent
  from home is recorded under the session that submission materialized — the
  first prompt of a new conversation is recallable from inside it.
...
- Storage is best-effort, matching `plugin-launcher-history.ts`: an unreadable
  ...
  a send. The source-level coverage in `composer-input-history.test.mjs` pins the
  per-conversation isolation, the caps, the browse sequence, the restored
  references, the keydown branch order (IME guard, autocomplete, history, send),
  the four record points, and the exit points; the real recall loop in the
  renderer and the rendered chips remain a Draft UI scenario
