...
| `ui.renderer` | high | Execute a declared renderer entry in the app UI realm | Derived from `manifest.renderer`; review on install or trust expansion | Same-realm trusted code; no per-slot grants. See [Composer references](17-composer-references.md) |
