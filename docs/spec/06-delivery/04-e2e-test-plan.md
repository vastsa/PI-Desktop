...
- **Steps**: 1) Ask the parent to review Frontend, Electron, and Rust in
  ...
  and durable worker sessions remain available. Repeat the parallel creation
  step with a large existing session list while keeping the parent visible.
- **Expected**: Each worker is a real durable session with the parent's
  ...
  fail closed. Bursts of worker create/prompt notifications serialize and
  coalesce session-list refreshes while preserving the final worker list and
  the foreground session. A later notification waits for a follow-up read;
  it is not lost by reusing a read that began before the worker was created.
- **Specs linked**: `07-plugins/03-plugin-api.md`,
