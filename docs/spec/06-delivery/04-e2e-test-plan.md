...
- **Expected**: The service is asked first and models.dev only enriches the answer. Discovery is debounced ~600 ms and a slow reply from an earlier keystroke never replaces a newer list; an unsaved provider is probed with the typed base URL and key before it exists. The user never types a token limit or picks an API format on the common path. Point a second provider at a base URL with no `/models` route and confirm the list falls back to the catalog, is labelled as coming from models.dev rather than the service, and still saves. The provider appears as a row with its host, model count and secret badge; the key is stored securely (not in plaintext config); `models` contains both bindings and `models[0]` remains the provider default.
