...
- **Steps**: 1) Open Settings → Model configuration and choose Add provider. 2) On the Provider stage, search the preset grid, confirm each card shows its published model count and that already-configured presets carry a Configured badge, then pick a preset. 3) On the Credential stage, confirm the name and base URL are prefilled from the published record, that a published base URL is shown read-only, and that the API format is not asked for — open Advanced and confirm it is present but derived. Enter an API key. 4) On the Models stage, confirm the browser lists only that preset's models, search by model ID and by display name, toggle the Reasoning/Vision/Tools/Attachments filters, and confirm the result count updates. 5) Drive the list with ArrowUp/ArrowDown and confirm the detail pane follows the active row, showing the published limits, pricing, modalities, thinking levels and knowledge cutoff as a labelled list. 6) Select two models with Enter and the mouse; confirm each appears in the chosen list with its published context/output already filled in and no numeric entry required. 7) Expand Advanced on one chosen row, override its limits and toggle thinking chips; confirm the other row is unaffected. 8) Add a free-form model ID that the catalog does not publish; confirm it is added with 128,000 / 8,192 / no-thinking defaults. Re-add the same ID in different letter case and confirm it is rejected as already added. 9) Save.
- **Expected**: The user never types a token limit or picks an API format on the common path: both come from the published record, with overrides behind per-row Advanced. Search is scored in the host (exact ID first, then prefix, substring, display name, family, provider) and a slow reply from an earlier keystroke never replaces a newer result set. The list is a keyboard-operable multi-select listbox with `aria-activedescendant`; the check column reserves its box so selection does not reflow the row. A model ID that differs only in letter case is treated as the same model by the check mark, the toggle and the duplicate guard. When the catalog cannot load, the browser says so and the custom-endpoint path plus manual model entry still work. The provider appears as a row with its host, model count and secret badge; the key is stored securely (not in plaintext config); `models` contains both bindings and `models[0]` remains the provider default.
...
- **Steps**: 1) Reopen the saved provider; confirm the dialog opens on the Credential stage rather than the preset grid, and that the API key field explains an unchanged key is kept. 2) Move to Models and confirm both bindings are still chosen with their limits, thinking chips and defaults intact, and that both are check-marked in the catalog list. 3) Edit one row's limits and save. 4) Reopen the fixture provider and confirm its legacy model appears as one chosen row with 128,000 context, 8,192 max output and no thinking levels. 5) Save the fixture provider without changing the model. 6) Make the edited provider the global default, reopen it, remove its first model so a different binding becomes the head, and save.
- **Expected**: Editing never drops an unmodified binding. A provider whose preset is no longer in the catalog still opens, falling back to the custom-endpoint treatment instead of failing. Legacy read materializes one binding without losing the old model ID; the subsequent write stores `config_json.models` and keeps `defaultModelId` equal to the first binding for older readers. When the edited provider is the global default and its first model changed, `settings.defaultModelId` is re-synced to the new head binding.
...
| M6+ | E2E-121, E2E-122, E2E-148, E2E-150, E2E-151, E2E-154, E2E-155, E2E-158, E2E-159, E2E-160 |
