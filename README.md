...
- **Background subagents.** Delegate separable work — wide searches, multi-file implementation, adversarial review — to built-in or your own subagents that run in their own context and report back, with per-model delegation choices and a bounded concurrency cap.
- **Agent-to-agent collaboration.** Coordinate concurrent subagents — and parent agents across open conversations — over the local Agent2Agent (A2A) protocol: discover running peers as Agent Cards, exchange durable tasks and typed messages, and stream task updates.
- **A real workbench.** Review the agent's edits as message-scoped diffs with guarded rollback, inspect command output in the transcript, preview a local app in a browser, and browse project files — all in internal side panels that reflow the chat, including views that plugins contribute.
...
- **A fast daily workflow.** Use slash commands and `@` file references, paste files as chips on the input line, queue the next prompt while a turn is still running, enhance a draft prompt in one shot, search everything globally with Option/Alt+Space, and create manual or recurring task prompts.
- **Resilient runs.** Transient provider failures and rate limits are retried with bounded backoff, streaming replies are checkpointed across quit and sidecar loss, and an interrupted answer can be continued instead of restarted.
- **Comfortable to live in.** English, 简体中文, and Türkçe, with searchable language and theme pickers, light/dark/system and plugin themes, a global search and command surface, a pickable UI font with bundled OFL faces, tray and close-behavior choices, onboarding checklist, local notifications, context checkpoints, and in-app release notes with update alerts for packaged builds.
...
PI-Desktop is an early preview under active development. The current 0.13.x line ships the app shell, streaming agent runtime, Agent/Plan/Goal contracts, workspace tools with permissions, the workbench, projects and sessions, imports, agent capability management (MCP/Skills/Subagents), background subagent delegation with local Agent2Agent (A2A) collaboration, discovery-driven provider setup with a models.dev-backed model catalog, multiple models and OAuth accounts per provider, extensions (plugins) with a marketplace, context checkpoints, notifications, in-app release notes, and cross-platform packaging with update delivery.
