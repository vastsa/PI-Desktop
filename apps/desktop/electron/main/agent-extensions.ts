 ...
 * files as `contributes.agentExtensions`, and a no-op `main.cjs`. A `.cjs`
 * wrapper stays CommonJS even when the copied `package.json` declares
 * `"type": "module"`, which would otherwise make Node evaluate the wrapper
 * as ESM and fail on `module.exports`. A directory
