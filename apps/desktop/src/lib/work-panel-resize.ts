 ...
 * a positive compact width must remain representable in persisted state and
 * over the reservation IPC seam.
 */
...
export function workPanelWidthForSidebarReopen({
  ...
  const mainWidth = Math.max(0, Math.round(currentMainWidth));
  const targetMainWidth = Math.max(MAIN_PANE_REOPEN_TARGET_WIDTH, mainWidth);
  return Math.max(
    WORK_PANEL_COMPACT_MIN_WIDTH,
    Math.min(panelWidth, width - leftWidth - targetMainWidth),
  );
}
