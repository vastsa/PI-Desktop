...
describe("resolvePromptEnhancementTemplates", () => {
  ...
  it("treats a blank override as 'not set'", () => {
    expect(
      resolvePromptEnhancementTemplates({ systemPrompt: "   ", userTemplate: "" }),
    ).toEqual({
      systemPrompt: PROMPT_ENHANCEMENT_DEFAULT_SYSTEM_PROMPT,
      userTemplate: PROMPT_ENHANCEMENT_DEFAULT_USER_TEMPLATE,
    });
    expect(
      resolvePromptEnhancementTemplates({ systemPrompt: null, userTemplate: null }),
    ).toEqual({
      systemPrompt: PROMPT_ENHANCEMENT_DEFAULT_SYSTEM_PROMPT,
      userTemplate: PROMPT_ENHANCEMENT_DEFAULT_USER_TEMPLATE,
    });
  ...
  it("uses stored overrides verbatim", () => {
  ...
  it("falls back for a user template that lost the draft variable", () => {
    ...
    const result = resolvePromptEnhancementTemplates({
      systemPrompt: "custom system",
      userTemplate: "no placeholder here",
    });
    expect(result.userTemplate).toBe(PROMPT_ENHANCEMENT_DEFAULT_USER_TEMPLATE);
    // A broken user template must not discard a valid system override.
    expect(result.systemPrompt).toBe("custom system");
  });
});
