...
importConfirm: "가져온 확장은 에이전트 프로세스 안에서 에이전트 자체 도구와 같은 권한으로 실행됩니다. 선언된 스킬은 에이전트에 지침을 추가할 수 있고, 선언된 의존성은 npm으로 설치됩니다(설치 스크립트 비활성화). 계속할까요?",
  ...
  index: {
    ...
    "grepBoostDesc": "이 스위치가 켜져 있을 때만 Grep이 이 워크스페이스의 인덱스에서 대소문자를 구분하는 리터럴 검색을 처리합니다. 다른 워크스페이스로 전환하면 그 인덱스를 백그라운드에서 만듭니다. 나머지는 rg로 자동 폴백합니다. 모든 데이터는 이 컴퓨터에 저장됩니다.",
    loadErrorTitle: "인덱스 상태를 사용할 수 없음",
    loadErrorDesc: "호스트가 응답하지 않습니다. 인덱스 없이도 Grep은 정상 동작합니다.",
    retry: "다시 시도",
    statusDesc: "인덱스는 다시 생성할 수 있는 로컬 캐시이며 Grep 결과는 이에 의존하지 않습니다.",
    indexSubtitle: "로컬 인덱스는 재구축 가능한 가속 캐시이며 Grep은 항상 rg 대체 경로를 유지합니다.",
    nudgeText: "두 기능 모두 로컬에서 실행되며 워크스페이스 내용을 업로드하지 않습니다.",
    nudgeDismiss: "확인",
    sectionCode: "코드베이스",
    fastPathHitRate: "빠른 경로 적중률",
    progressFiles: "{{done}} / {{total}}개 파일",
    progressFallback: "Grep은 자동으로 rg로 대체되며 검색은 영향을 받지 않습니다.",
    emptyTitle: "현재 워크스페이스에 인덱스가 없습니다",
    emptyDesc: "생성하면 파일 수와 크기를 볼 수 있습니다. 인덱스는 이 컴퓨터에만 저장되며, 인덱스 가속이 켜져 있을 때만 읽힙니다.",
    actions: "인덱스 작업",
    actionsDesc: "현재 워크스페이스의 인덱스를 만들거나 다시 만듭니다. 인덱스 가속이 꺼져 있으면 만들 수 없습니다. 읽을 주체가 없기 때문입니다.",
    localOnly: "모든 데이터는 로컬에 저장되며 업로드되지 않습니다.",
    rebuilding: "색인 중…",
    clearing: "삭제 중…",
    actionError: "작업이 실패했습니다. 다시 시도할 수 있으며 Grep에는 영향이 없습니다.",
    card: {
      health: "인덱스 상태",
      status: "상태",
      files: "색인된 파일",
