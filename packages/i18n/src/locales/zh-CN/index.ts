...
sklm: {
    ...
    permissionDeny: "拒绝规则",
    permissionDenyDesc: "始终拒绝的 JSON glob，全自动模式也会生效。paths 匹配 Read/Write/Edit/Glob/Grep 的 path/file_path，会解析 ..、~ 和符号链接，但不覆盖 Bash 命令文本或 Grep 搜到的内容。commands 是去掉首尾空白后的 Bash 字符串前缀或 glob，不是 argv。插件只能追加规则，不能放宽你的设置。",
