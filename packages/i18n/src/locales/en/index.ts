...
importConfirm: "Imported extensions run inside the agent process with the same access as the agent's own tools. Declared skills can add instructions to the agent, and declared dependencies are installed with npm (install scripts disabled). Continue?",
  ...
  index: {
    ...
    grepBoostDesc: "While this switch is on, Grep may serve case-sensitive literal searches from this workspace's index, and switching workspace builds the next one in the background. Everything else falls back to rg. Everything stays on this machine.",
    loadErrorTitle: "Index status unavailable",
    loadErrorDesc: "The host did not answer. Grep works without the index.",
    retry: "Retry",
    statusDesc: "The index is a rebuildable local cache; Grep results never depend on it.",
    indexSubtitle: "The local index is a rebuildable acceleration cache; Grep always keeps its rg fallback.",
    nudgeText: "Both features run locally and never upload your workspace content.",
    nudgeDismiss: "Dismiss",
    sectionCode: "Codebase",
    fastPathHitRate: "Fast-path hit rate",
    progressFiles: "{{done}} / {{total}} files",
    progressFallback: "Grep falls back to rg automatically; search is unaffected.",
    emptyTitle: "No index for the current workspace",
    emptyDesc: "Build one to see file counts and size. The index stays on this machine and is only read while the index boost is on.",
    actions: "Index actions",
    actionsDesc: "Build or rebuild the index for the current workspace. It cannot be built while the index boost is off, because nothing would read it.",
    localOnly: "Everything is stored locally and never uploaded.",
    rebuilding: "Indexing…",
    clearing: "Clearing…",
    actionError: "The index action failed. You can retry; Grep is unaffected.",
    card: {
      health: "Index health",
      status: "Status",
      files: "Indexed files",
    ...
    status: {
      ...
      partial: "Partial",
      disabled: "Off",
      skipped_over_limit: "Over budget",
    },
  },
} as const;
