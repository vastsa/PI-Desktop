...
export const PLAN_MODE_SYSTEM_PROMPT = [
  ...
  "Do not write or edit a plan file yourself. When any initial or revised plan is ready, call `submit_plan` immediately exactly once in the current turn with one complete Markdown snapshot, a title, and the question that needs approval; the host writes a new .pi/plan artifact and opens the review.",
  "An accepted new Plan prompt means no prior approval is pending. Earlier `submit_plan` calls in the transcript are historical immutable checkpoints, not the current plan and not an active approval gate.",
  "After reject, expiry, or interruption closes approval and returns to editable planning, revise the plan in the new turn and follow the same one-`submit_plan` rule; never edit or replace an earlier artifact.",
...
export const GOAL_MODE_SYSTEM_PROMPT = [
  ...
  "Do not write or edit a goal file yourself. When the goal, its acceptance criteria, and its boundaries are ready, call `submit_goal` immediately exactly once in the current turn with one complete Markdown snapshot, a title, and the question that needs approval; the host writes a new .pi/goal artifact and opens the review.",
  "An accepted new Goal prompt means no prior approval is pending. Earlier `submit_goal` calls in the transcript are historical immutable checkpoints, not the current contract and not an active approval gate.",
  "After reject, expiry, or interruption closes approval and returns to editable goal negotiation, revise the contract in the new turn and follow the same one-`submit_goal` rule; never edit or replace an earlier artifact.",
